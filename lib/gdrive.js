...
async function driveGet(url, params, responseType, attempt = 1) {
  ...
  try {
    ...
    // Transient failures (connection resets, timeouts, 429/5xx) are retried
    // with a short backoff so one network hiccup never breaks a download.
    const status = e && e.response && e.response.status;
    const retryable = !e.response || status === 429 || (status >= 500 && status <= 504);
    if (retryable && attempt < 3) {
      console.warn(`⚠️ gdrive: transient error (${e.code || status || 'network'}), retry ${attempt + 1}/3…`);
      await sleep(600 * attempt);
      return driveGet(url, params, responseType, attempt + 1);
    }
    throw e;
  }
}
...
async function getMeta(id) {
  return driveGet(`${API_BASE}/files/${id}`, { fields: 'id,name,mimeType,size' });
}
...
function friendlyError(e) {
  ...
  if (!status && e && ['ECONNRESET', 'ECONNABORTED', 'ETIMEDOUT', 'EAI_AGAIN', 'ECONNREFUSED', 'EPIPE'].includes(e.code)) {
    return '📡 Google Drive was unreachable for a moment (network hiccup). Please try again shortly.';
  }
  if (status === 403) {
