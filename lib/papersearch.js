...
async function aiInterpret(query, index) {
  ...
  const prompt =
    ...
    '{"action":"find","year":<number or null>,"subject":"<subject code or null>","medium":"<sinhala|english|tamil or null>","type":"<marking|mcq|essay|paper or null>","category":"<past or null>"}\n' +
    ...
    '"category" is "past" (or null) — the library holds plain A/L exam papers and marking schemes only.\n'
...
const SUBJECTS = {
  ...
  hinduism:        { label: 'Hinduism', aliases: ['hinduism', 'හින්දු'] },
  hinducivilization: { label: 'Hindu Civilization', aliases: ['hindu civil', 'hindu civ'] },
  islamicivilization: { label: 'Islam Civilization', aliases: ['islam civil', 'islamic civilization', 'islamic civ'] },
  christiancivilization: { label: 'Christian Civilization', aliases: ['christian civil', 'christian civ'] },
  german:          { label: 'German', aliases: ['german', 'ජර්මන්'] },
  russian:         { label: 'Russian', aliases: ['russian', 'රුසියන්'] },
  chinese:         { label: 'Chinese', aliases: ['chinese', 'චීන'] },
  korean:          { label: 'Korean', aliases: ['korean', 'කොරියානු'] },
  agrotechnology:  { label: 'Agro Technology', aliases: ['agro tech', 'agricultural technology'] },
  bioresourcetechnology: { label: 'Bio-Resource Technology', aliases: ['bio resource', 'bio-resource', 'brt'] },
  electronictech:  { label: 'Electrical, Electronic and Information Technology', aliases: ['electrical electronic', 'electronic and information'] },
  commongeneraltest: { label: 'Common General Test', aliases: ['common general test', 'cgt'] },
  generalenglish:  { label: 'General English', aliases: ['general english'] },
  highermathematics: { label: 'Higher Mathematics', aliases: ['higher maths', 'further maths'] }
};
...
/* PAPER COLLECTIONS — plain A/L papers only (a file without a marker
   counts as the regular past paper). FWC and Provincial collections
   were removed from the Drive library. */
const CATEGORIES = {
...
// words that neither add meaning nor disqualify a file from being a plain
// past paper — includes the retired collection names (FWC/Provincial)
const FILLER_WORDS = new Set(['paper', 'papers', 'fwc', 'provincial', 'provinsial', 'provencial', 'province', 'palath', 'palathe', 'al', 'a l']);

/**
 * Strictly classify a file name in "Year + Subject + Medium" format.
 * Returns { year, subject, medium, cat, extra } where `cat` is the paper
 * collection ("past" — unmarked = past) and `extra` holds
 ...
 * (Marking) is recognised from folder names too.
 */
...
function matchPaper(index, q) {
  ...
  for (const f of (index.files || [])) {
    ...
    if (wantCat && c.cat !== wantCat) continue;      // collection filter
...
/**
 * EVERY subject that appears anywhere in the library — file names, folder
 * names and path segments, matched by label or any alias (Sinhala/Tamil
 * included). This is the real-time subject inventory of the Drive.
 * Ultra-short aliases (it, cm, et, …) only count in ALL-CAPS form so
 * ordinary English words never fake a subject.
 */
function subjectsInIndex(index) {
  const rawParts = [];
  for (const f of ((index && index.files) || [])) {
    rawParts.push(String(f.name || ''));
    for (const p of (f.path || [])) rawParts.push(String(p));
  }
  for (const fo of ((index && index.folders) || [])) {
    rawParts.push(String(fo.name || ''));
    for (const p of (fo.path || [])) rawParts.push(String(p));
  }
  if (!rawParts.length) return [];
  const raw = rawParts.join(' \n ').replace(/\u200D/g, '').replace(/[_\-./]+/g, ' ');
  const lower = raw.toLowerCase();
  const found = new Map();   // key -> where it was found ('name' | 'files')
  // sinhala/tamil/english are ALSO medium names ("_Sinhala.pdf") — they only
  // count as SUBJECTS when a file's classification says so, never from the
  // broad text scan (else every file would fake a Sinhala subject)
  const LANG_SUBJECTS = new Set(['sinhala', 'tamil', 'english']);
  for (const c of classifyAll(index).values()) {
    if (LANG_SUBJECTS.has(c.subject)) found.set(c.subject, 'files');
  }
  for (const [k, v] of Object.entries(SUBJECTS)) {
    if (LANG_SUBJECTS.has(k)) continue;
    const variants = [v.label, k, ...(v.aliases || [])]
      .map((s) => String(s).toLowerCase().replace(/[_\-./]+/g, ' ').trim())
      .filter(Boolean);
    const hit = variants.some((a) => {
      if (a.length <= 2) {
        return new RegExp('(^|[^A-Za-z])' + _escRe(a.toUpperCase()) + '([^A-Za-z]|$)').test(raw);
      }
      return new RegExp('(^|[^\\p{L}\\p{M}\\p{N}])' + _escRe(a) + '([^\\p{L}\\p{M}\\p{N}]|$)', 'u').test(lower);
    });
    if (hit && !found.has(k)) found.set(k, 'name');
  }
  // SAFETY NET — subject folders the vocabulary doesn't know still COUNT.
  // Any folder that isn't a year / medium / generic-noise name is a subject
  // (the Drive layout is Year > Subject > medium). This keeps the count
  // truthful even for brand-new subjects nobody added to the vocabulary.
  const NOISE_FOLDERS = new Set(['papers', 'paper', 'past', 'past papers', 'pastpapers', 'marking', 'markings',
    'marking scheme', 'marking schemes', 'scheme', 'schemes', 'mcq', 'mcqs', 'essay', 'essays', 'structured',
    'notes', 'note', 'tute', 'tutes', 'tutorials', 'tutorial', 'resource', 'resources', 'revision', 'model',
    'model papers', 'new syllabus', 'old syllabus', 'syllabus', 'new', 'old', 'question', 'questions',
    'question papers', 'answers', 'answer', 'test', 'exam', 'exams', 'medium', 'sheet', 'sheets',
    'assignment', 'assignments', 'time table', 'timetable', 'other', 'others', 'misc', 'verydeep']);
  const MED_FOLDERS = new Set(['sinhala', 'english', 'tamil', 'sinhala medium', 'english medium', 'tamil medium']);
  const variantsOfFound = [...found.keys()].map((k) => [SUBJECTS[k].label, k, ...(SUBJECTS[k].aliases || [])]
    .map((s) => String(s).toLowerCase().replace(/[_\-./]+/g, ' ').trim()));
  const entries = [...found].map(([k, s]) => ({ key: k, label: SUBJECTS[k].label, src: s }));
  const seenLabels = new Set(entries.map((e) => e.label.toLowerCase()));
  for (const fo of ((index && index.folders) || [])) {
    // normalize: strip leading emoji/number decorations, unify separators,
    // collapse whitespace ("1️⃣ A/L Past Papers", "ENGINEERING  TECHNOLOGY")
    const name = String(fo.name || '').replace(/\u200D/g, '')
      .replace(/^[^\p{L}\p{M}\p{N}0-9]+/u, '')
      .replace(/[_\-]+/g, ' ').replace(/\s+/g, ' ').trim();
    if (!name || name.length < 3) continue;
    const low = name.toLowerCase();
    if (/^(19|20)\d{2}$/.test(low.replace(/\s+/g, ''))) continue;
    if (NOISE_FOLDERS.has(low) || MED_FOLDERS.has(low)) continue;
    if (low.includes('past paper')) continue;                    // "A/L Past Papers" etc
    if (low === 'languages' || low === 'common general') continue;
    // "… Technology" folders are STREAMS here (Civil/Electronic/Food/
    // Mechanical); every REAL technology subject is in the vocabulary,
    // so unknown "* technology" folders never count
    if (/technology$/.test(low) && !seenLabels.has(low)) continue;
    if (seenLabels.has(low)) continue;
    // already covered by a known subject via its name/aliases?
    if (variantsOfFound.some((vs) => vs.some((v) => v.length >= 4
      ? low.includes(v)
      : new RegExp('(^|[^A-Za-z])' + _escRe(v.toUpperCase()) + '([^A-Za-z]|$)').test(name)))) continue;
    entries.push({ key: null, label: name, src: 'folder' });
    seenLabels.add(low);
  }
...
module.exports = {
  ...
  CATEGORIES, TYPE_WORDS, classifyAll, subjectsInIndex
};
