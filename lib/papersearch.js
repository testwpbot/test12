...
async function aiInterpret(query, index) {
  ...
  const prompt =
    ...
    '{"action":"find","year":<number or null>,"subject":"<subject code or null>","medium":"<sinhala|english|tamil or null>","type":"<marking|mcq|essay|paper or null>"}\n' +
    '{"action":"search","keywords":"2-6 simple english keywords"}\n' +
    '{"action":"none"}  // message is not about getting papers\n' +
    '"type" is what the student wants: marking scheme -> "marking", MCQ paper -> "mcq", essay/structured -> "essay", the question paper itself -> "paper" or null.\n'
...
function normalizeInterpretation(obj) {
  ...
  const out = { action, year: null, subject: null, medium: null, type: null, keywords: '' };
  if (action === 'find') {
...
const SUBJECTS = {
  ...
  agriculture:    { label: 'Agriculture', aliases: ['agri', 'agro', 'agric', 'agricultural', 'agricultural science', 'agri science', 'farming'] },
...
const MEDIUMS = {
  sinhala: { label: 'Sinhala', tokens: ['sinhala', 'sin', 'si', 'sinhla', 'sinhla', 'සිංහල'] },
  english: { label: 'English', tokens: ['english', 'eng', 'en', 'ඉංග්රීසි'] },
...
function parsePaperQuery(text) {
  ...
  if (!subject && !medium && !hasMediumNoun) return null;  // nothing usable
