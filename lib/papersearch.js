...
function subjectsInIndex(index) {
  ...
  for (const c of classifyAll(index).values()) {
    if (LANG_SUBJECTS.has(c.subject)) found.add(c.subject);
  }
  ...
  const variantsOfFound = [...found].map((k) => [SUBJECTS[k].label, k, ...(SUBJECTS[k].aliases || [])]
    .map((s) => String(s).toLowerCase().replace(/[_\-./]+/g, ' ').trim()));
  const entries = [...found].map((k) => ({ key: k, label: SUBJECTS[k].label }));
  ...
  for (const fo of ((index && index.folders) || [])) {
    ...
    entries.push({ key: null, label: name });
    seenLabels.add(low);
  }
