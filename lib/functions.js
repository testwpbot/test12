...
/**
 * Normalise a group-participant entry into { digits, jid }.
 * Baileys 7 rc emits objects ({ id, phoneNumber, lid, admin }) where the id
 * is often a @lid JID; older versions emitted plain JID strings. Prefers the
 * real phone-number JID for both the digits and the mention target.
 */
function participantInfo(p) {
  const raws = typeof p === 'string' ? [p] : [p && p.phoneNumber, p && p.id, p && p.lid, p && p.pn].filter(Boolean).map(String);
  let digits = '', jid = '';
  for (const raw of raws) {
    const d = String(raw).split('@')[0].split(':')[0].replace(/\D/g, '');
    if (!digits && d) digits = d;
    if (!jid && /@s\.whatsapp\.net$/.test(raw)) jid = raw;
  }
  if (!jid) jid = raws[0] || '';
  return { digits, jid };
}
