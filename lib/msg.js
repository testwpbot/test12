...
const sms = (danuwa, m) => {
	...
	m.sendButtonMenu = (config, options = {}) => sendButtonMenu(danuwa, m.chat, config, { ...(isTapResponse(m) ? {} : { quoted: m }), ephemeralExpiration: m.message?.ephemeralMessage?.contextInfo?.expiration, ...options })
