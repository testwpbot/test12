...
async function sendButtonMenu(sock, jid, config = {}, options = {}) {
  ...
  // plain text on every client (see sendLegacyList). Legacy lists have no
  // image header — the logo is intentionally skipped here.
  if (sections && Array.isArray(sections)) {
    return await sendLegacyList(sock, jid, {
      title,
      text,
      footer,
      listTitle,
