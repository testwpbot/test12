...
async function sendLegacyList(sock, jid, data = {}, options = {}) {
  ...
  // REQUIRED: the <biz> list stanza declares the payload type to WhatsApp's
  // server. Without it the relay ACKs locally but the message is silently
  // dropped and never reaches the chat. Node shape from Evolution API 2.4.0
  // (production-verified for legacy listMessage cross-client rendering).
  const listBizNode = {
    tag: 'biz',
    attrs: {},
  ...
  try {
    await sock.relayMessage(jid, msg.message, { messageId: msg.key.id, additionalNodes: [listBizNode] });
  } catch (e) {
